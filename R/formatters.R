# Class-aware input formatters

#' Turn an R object into LLM-friendly text
#'
#' @param x Any R object.
#' @param ... Additional arguments passed to methods.
#' @return A character string suitable to send as model input.
#' @export
...
#' @export
format_input.character <- function(x, ...) {
  paste(x, collapse = "\n")
}
...
format_input.matrix <- function(x, head_n = 10, ...) {
  ...
  paste0(
    ...
    "- type: ", typeof(x), "\n\n",
    "## top-left corner (", head_n, "x", head_n, ")\n",
...
#' @export
format_input.Seurat <- function(x,
                                meta_cols = c("orig.ident", "seurat_clusters"),
                                assays = NULL,
                                ...) {
  has_seurat <- requireNamespace("Seurat", quietly = TRUE)
  if (!has_seurat) return(format_input.default(x))
